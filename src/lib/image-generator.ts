...
const CIRCULAR_PHOTO_RADIUS = 100;
...
function drawBmmHeader(
  ...
  if (circular) {
    const logoTopY = layoutY(layout, scaleCoordY(58, canvasH), canvasH);
    drawLogoAt(
      ctx,
      logo,
...
async function drawBmmPersonalPoster(
  ...
  if (circular) {
    const nameY =
      ...
      photoRadius +
      ringPadding +
      layoutScale(layout, 36, POSTER_W);
...
async function drawPersonalDp(
  ...
  if (circular) {
    const nameY =
      ...
      photoRadius +
      ringPadding +
      layoutScale(layout, scaleCoord(36, DP_W), DP_W);
    drawAttendeeBlockCentered(
      ctx,
      displayName,
      input.role,
